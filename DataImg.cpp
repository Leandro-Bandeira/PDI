...
void DataImg::applyPontYIQRGB(std::string saveImg)
{
    ...
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < cols; j++)
        {
            ...
            new_B = std::min(std::max(new_B, 0.0), 255.0);
            new_G = std::min(std::max(new_G, 0.0), 255.0);
            new_R = std::min(std::max(new_R, 0.0), 255.0);
...
void DataImg::applyFilters(tMask filters, int isSobel, std::string saveImg)
{
    ...
    std::vector<std::vector<double>> mask = filters.mask;
    int totalLineMask = filters.m;
    int totalColsMask = filters.n;
    ...
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < cols; j++)
        {
            ...
            else
            {
                ...
                sumPixel[0] = std::min(std::max(sumPixel[0], 0.0), 255.0) + filters.offset;
                sumPixel[1] = std::min(std::max(sumPixel[1], 0.0), 255.0) + filters.offset;
                sumPixel[2] = std::min(std::max(sumPixel[2], 0.0), 255.0) + filters.offset;
            }
    ...
    if (isSobel == 1)
    {
        ...
        for (int i = 0; i < rows; i++)
        {
            ...
            for (int j = 0; j < cols; j++)
            {
                ...
                sumPixel[0] = std::round(255 * ((static_cast<double>(sumPixel[0]) - minVals[0]) / (maxVals[0] - minVals[0]))) + filters.offset;
                sumPixel[1] = std::round(255 * ((static_cast<double>(sumPixel[1]) - minVals[1]) / (maxVals[1] - minVals[1]))) + filters.offset;
                sumPixel[2] = std::round(255 * ((static_cast<double>(sumPixel[2]) - minVals[2]) / (maxVals[2] - minVals[2]))) + filters.offset;
